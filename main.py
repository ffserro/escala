...
if mes != 0:
    ...
    with st.form('indisponibilidade'):
        ...
        if send_ind:
            st.session_state.indisponivel[mil_ind+str(list(st.session_state.indisponivel.keys()).count(mil_ind))] = [mot_ind] + [per_ind[0] + timedelta(n) for n in range((per_ind[-1] - per_ind[0]).days+1)]
